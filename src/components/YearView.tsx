          ...
          return (
            ...
            >
              ...
              >
                {/* 月名 */}
                <div
                  className="w-10 font-mono text-sm font-bold shrink-0"
                  style={{ color: isCurrentMonth ? '#00f5ff' : '#888' }}
                >
                  {MONTH_NAMES[month - 1]}
                </div>
                ...
                <div className="flex-1 relative h-5 rounded overflow-hidden" style={{ background: '#111' }}>
                  ...
                  {totalMins === 0 && !isFuture && (
                    <div className="absolute inset-0 flex items-center pl-2">
                      <span className="text-xs font-mono text-gray-800 tracking-widest">
                        NO DATA
                      </span>
                    </div>
                  )}
                ...
                <div className="w-20 text-right font-mono text-xs shrink-0">
                  {totalMins > 0 ? (
                    <span style={{ color: isCurrentMonth ? '#00f5ff' : '#39ff14' }}>
                ...
                {/* 件数 */}
                <div className="w-10 text-right font-mono text-xs shrink-0 text-gray-700">
                  {count > 0 ? `${count}件` : ''}
                </div>

                {/* 矢印 */}
                <div className="font-mono text-gray-700 group-hover:text-gray-400 transition-colors text-sm shrink-0">
                  ›
                </div>
              </div>
            </button>
